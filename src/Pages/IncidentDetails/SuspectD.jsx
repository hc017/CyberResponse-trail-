...
const SuspectD = () => {
  ...
  const handleAddSuspect = async () => {
    ...
    try {
      ...
      // Get the ID generated by Firebase
    const newSuspectId = newSuspectRef.key;

    // Construct the final ID with the prefix 'susdata'
    const susDataId = `susdata${newSuspectId}`;

    // Store suspect data in Realtime Database with the unique ID
    await set(ref(db, `users/${currentUser.uid}/incidentdetails/${incidentId}/suspects/${susDataId}`), data);
