...
const SuspectD = () => {
  ...
  const handleSaveAndSubmit = async () => {
    ...
    try {
      await Promise.all(
        suspectData.map(async (suspect) => {
          const newSuspectRef = push(ref(db, `users/${currentUser.uid}/incidentdetails/${incidentId}/suspects`));
          const newSuspectId = newSuspectRef.key;
          const susDataId = `susdata${newSuspectId}`;
          await set(ref(db, `users/${currentUser.uid}/incidentdetails/${incidentId}/suspects/${susDataId}`), suspect);
        })
      );
      ...
      // Clear suspect data after saving to Firebase
      setSuspectData([]);
      setShowTable(false);
          ...
          {isCheckboxChecked && (
            <div>
              ...
              <div className="suspect_bock2">
                ...
                <button className="SS_button" id="ss_upload" onClick={handleFileUpload}>Upload</button>
              </div>
            </div>
          )}
